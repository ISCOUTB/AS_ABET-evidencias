...
def get_outcomes():
    ...
    cursor.execute("SELECT id, so_number, description_es AS description FROM mdl_gradingform_utb_so")
...
def get_indicators(outcome_id: str):
    ...
    try:
        ...
        cursor.execute("SELECT id, student_outcome_id, indicator_letter, description_es AS description FROM mdl_gradingform_utb_ind WHERE student_outcome_id = %s", (outcome_id_int,))
        results = cursor.fetchall()
        return results
...
def get_outcome_summary(outcome_id: int):
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT * FROM mdl_gradingform_utb_so WHERE id = %s", (outcome_id,))
        outcome = cursor.fetchone()
        if not outcome:
            raise HTTPException(status_code=404, detail="Outcome no encontrado")
        cursor.execute("SELECT * FROM mdl_gradingform_utb_ind WHERE student_outcome_id = %s", (outcome_id,))
...
def get_outcome_assessment(outcome_id: int):
    ...
    try:
        ...
        cursor.execute("SELECT id, so_number FROM mdl_gradingform_utb_so WHERE id = %s", (outcome_id,))
        outcome = cursor.fetchone()
        if not outcome:
